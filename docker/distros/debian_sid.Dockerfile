...
RUN apt-cache search llvm.*tools
RUN apt-get update && apt-get upgrade -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" && apt-get install -y llvm coreutils util-linux yq file expect sudo git make tar jq build-essential locales tzdata wget zstd && apt-get clean
